...
public class TrueTimeRx
  ...
  /**
   * Initialize TrueTime See {@link #initializeNtp(String)} for details on working
   *
  ...
  /**
   * Initialize TrueTime A single NTP pool server is provided. Using DNS we resolve that to multiple IP hosts (See
   * {@link #initializeNtp(List)} for manually resolved IPs)
   * <p>
  ...
  /**
   * Initialize TrueTime Use this if you want to resolve the NTP Pool address to individual IPs yourself
   * <p>
   * See https://github.com/instacart/truetime-android/issues/42 to understand why you may want to do something like
   * this.
   *
  ...
  /**
   * Transformer that takes in a pool of NTP addresses Against each IP host we issue a UDP call and retrieve the best
   * response using the NTP algorithm
   */
